                    ...
                    <div class="py-3">
                        <div class="row">
                            <div class="col">
                                ...
                                <button type="button" class="btn btn-outline-success">Đơn hàng đã giao:<?= $sldg ?> </button>
                            </div>
                            <div class="col">
                                <?php
                                $sqlvc = "SELECT * FROM `hoadon` WHERE `trangthai`= 'Đang vận chuyển'";
                                $slvc = $conn->query($sqlvc)->num_rows; ?>
                                <button type="button" class="btn btn-outline-warning">Đơn hàng đang vận chuyển:<?= $slvc ?> </button>
                            </div>
                            <div class="col">
                                <?php
                                ...
                                <tbody>
                                    ...
                                    ?>
                                            <tr>
                                                ...
                                                <td><a href="../chitiethoadon.php?mhd=<?= $row["mahoadon"]?>"><?= $row["giatri"] ?></a></td>
                                                <td><?= $row["thoigian"] ?></td>
                                                <?php
                                                if ($row["trangthai"] == "Đã nhận hàng") {
                                                ?>
                                                    <td><button type="button" class="btn btn-outline-success" data-bs-toggle="modal" data-bs-target="#a<?= $row["stt"] ?>"><?= $row["trangthai"] ?></button></td>
                                                <?php
                                                } elseif ($row["trangthai"] == "Đang vận chuyển") {
                                                ?>
                                                    <td><button type="button" class="btn btn-outline-warning" data-bs-toggle="modal" data-bs-target="#a<?= $row["stt"] ?>"><?= $row["trangthai"] ?></button></td>
                                                <?php
                                                } else {
                                                ?>
                                                    <td><button type="button" class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#a<?= $row["stt"] ?>"><?= $row["trangthai"] ?></button></td>
                                                <?php
                                                }
                                                ?>
                                            </tr>
                                            <div class="modal fade" id="a<?= $row["stt"] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-5" id="exampleModalLabel">Cập nhật thông tin đơn hàng</h1>
                                                        </div>
                                                        <div class="modal-body">
                                                            <form method="POST" action="../admin/xulyadmin.php">
                                                                <input type="hidden" name="mahoadon" value="<?= $row["mahoadon"] ?>">
                                                                <div class="form-check">
                                                                    <input class="form-check-input" type="radio" name="trangthai" id="flexRadio2" value="Đang vận chuyển">
                                                                    <label class="form-check-label" for="flexRadio2">
                                                                        Đang vận chuyển
                                                                    </label>
                                                                </div>
                                                                <div class="form-check">
                                                                    <input class="form-check-input" type="radio" name="trangthai" id="flexRadio3" value="Đã nhận hàng">
                                                                    <label class="form-check-label" for="flexRadio3">
                                                                        Đã nhận hàng
                                                                    </label>
                                                                </div>
                                                                <div class="form-check">
                                                                    <input class="form-check-input" type="radio" name="trangthai" id="flexRadio3" value="Hủy">
                                                                    <label class="form-check-label" for="flexRadio3">
                                                                        Hủy đơn hàng
                                                                    </label>
                                                                </div>
                                                                <div class="modal-footer">
                                                                    ...
                                                                    <div class="col"><button type="submit" class="btn btn-primary" name="confirm">xác nhận</button></div>
                                                                </div>
                                                            </form>
                                                        </div>
                                                    </div>
